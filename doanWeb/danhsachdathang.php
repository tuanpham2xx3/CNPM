...
if(isset($_POST['send'])) {
    try {
        ...
        $mail->SMTPAuth = true;                             
        $mail->Username = 'abc@vku.udn.vn';               
        $mail->Password = 'abc';                         
        $mail->SMTPSecure = 'tls';                            
        $mail->Port = 587;                                    
        $mail->CharSet="UTF-8";
        $mail->setFrom('abc', 'BaloTT.com');
        $mail->addAddress($_POST['email'], $_POST['name']);    

        $mail->isHTML(true);                                 
        $mail->Subject = "Xác nhận đơn hàng thành công";
        $mail->Body    = " Xin chào quý khách hàng:  <br>
                            Cảm ơn quý khách đã mua sản phẩm tại website: BaloTT.com <br>";
        $mail->send();
        echo "<script>alert('Email xác nhận đặt hàng đã gửi')</script>";
    } 
    catch (Exception $e) 
    {
...
<html lang="vi">
<head>
    <title>BaloTT Balo chính hãng chất lượng cao</title>
