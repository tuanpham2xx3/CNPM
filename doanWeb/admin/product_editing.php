        ...
        <div id="content-box">
            ...
            if (isset($_GET['action']) && ($_GET['action'] == 'add' || $_GET['action'] == 'edit')) {
                if (isset($_POST['name']) && !empty($_POST['name']) && isset($_POST['price']) && !empty($_POST['price'])) {
                    ...
                    if (!isset($error)) {
                        if ($_GET['action'] == 'edit' && !empty($_GET['id'])) { //Cập nhật lại sản phẩm
                            $result = mysqli_query($con, "UPDATE `product` SET `menu_id` = '" . $_POST['menu_id'] . "', `name` = '" . $_POST['name'] . "' ,`quatity` = '" . $_POST['quatity'] . "' , `image` =  '" . $image . "', `price` = " . str_replace('.', '', $_POST['price']) . ", `price_new` = " . str_replace('.', '', $_POST['price_new']) . ", `content` = '" . $_POST['content'] . "', `last_updated` = " . time() . " WHERE `product`.`id` = " . $_GET['id']);
                        } else { //Thêm sản phẩm
                            $result = mysqli_query($con, "INSERT INTO `product` (`id`,`menu_id`, `name`, `quatity` , `image`, `price`,`price_new`, `content`, `created_time`, `last_updated`) VALUES (NULL, '" . $_POST['menu_id'] . "','" . $_POST['name'] . "','" . $_POST['quatity'] . "','" . $image . "', " . str_replace('.', '', $_POST['price']) . "," . str_replace('.', '', $_POST['price_new']) . ", '" . $_POST['content'] . "', " . time() . ", " . time() . ");");
                        }
                ...
                <form id="product-form" method="POST" action="<?= (!empty($product) && !isset($_GET['task'])) ? "?action=edit&id=" . $_GET['id'] : "?action=add" ?>"  enctype="multipart/form-data">
                    ...
                    <div class="wrap-field">
                        <label>Tồn kho: </label>
                        <input type="text" name="quatity" value="<?= (!empty($product) ? number_format($product['quatity'], 0, ",", ".") : "") ?>" />
                        <div class="clear-both"></div>
                    </div>
